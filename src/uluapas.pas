...
uses
  ...
  DCBasicTypes, Variants, uFile, uFileSource, uFileSystemFileSource,
  uFileProperty, uFileFunctions, uFileSourceUtil, uFileSourceProperty,
...
begin
  ...
  begin
    if gWdxPlugins.IsLoaded(AName) then
      bTry := True
    else begin
      if gWdxPlugins.LoadModule(AName) then
        bTry := True;
    end;
  end;
  if bTry then
